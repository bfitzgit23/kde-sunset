...
# @DESCRIPTION:
# SCM to use_with if this is a live ebuild.
: ${KDE_SCM:=git}
...
# @INTERNAL
# @DESCRIPTION:
# Determine if the current GCC version is acceptable, otherwise eerror.
_check_gcc_version() {
	if [[ ${MERGE_TYPE} != binary && -v KDE_GCC_MINIMAL ]] && tc-is-gcc; then
		...
		[[ ${major} -lt ${min_major} ]] || \
			...
			&& eerror "Sorry, but gcc-${KDE_GCC_MINIMAL} or later is required for this package (found ${version})."
	fi
}
...
_add_category_dep() {
	...
	echo " ${operator}${category}/${package}${version}${slot}${use_with}"
}
...
# version to use_with instead of the automatic version (use_with sparingly). In addition,
# the optional fourth argument defines slot+operator instead of automatic slot
# (use_with even more sparingly).
...
add_frameworks_dep() {
	...
	if [[ $# -gt 4 ]]; then
		eerror "${FUNCNAME} was called with too many arguments"
	fi
...
punt_bogus_dep() {
	...
	sed -e "${first},${last}s/${dep}//" -i CMakeLists.txt || eerror

	if [[ ${length} = 1 ]] ; then
		sed -e "/find_package\s*(\s*${prefix}\(\s\+\(REQUIRED\|CONFIG\|COMPONENTS\|\${[A-Z0-9_]*}\)\)\+\s*)/Is/^/# removed by kde5-functions.eclass - /" -i CMakeLists.txt || eerror
	fi
}
